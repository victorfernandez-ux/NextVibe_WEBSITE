(function() {
    ...
    // Typewriter rotation for audience segments (hero subtitle + services header)
    (function() {
        var reduceMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;

        function initTypewriter(el, focusesKey, fallbackFocuses) {
            var index = 0;
            var timer = null;
        ...
        var heroEl = document.getElementById('heroFocus');
        if (heroEl) initTypewriter(heroEl, 'hero.focuses',
            ['UK Recruitment Firms', 'Marketing Agencies', 'Coaches & Consultants', 'E-Commerce Brands']);

        var servicesEl = document.getElementById('servicesFocus');
        if (servicesEl) initTypewriter(servicesEl, 'services.focuses',
            ['Recruitment Desk', 'Small Business', 'Marketer', 'Consultant']);
    })();
